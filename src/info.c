...
uint64_t *get_load_status() {
  ...
  uint64_t vars[7];
  sscanf(buf, "cpu %d %d %d %d %d %d %d", &vars[0], &vars[1], &vars[2],
...
uint8_t *get_load(uint16_t rate) {
  ...
  float percent =
      ((float)(end[1] - start[1])) / ((float)(end[0] - start[0])) * 100.0;
  free(end);
  free(start);
  static uint8_t str[20];
...
uint8_t *get_core_temp() {
  ...
  if (!fp) {
    perror(i18n("Error:open thermal file failed"));
    exit(1);
  }
...
uint64_t get_network_status(const uint8_t *ifname, const uint8_t *txorrx) {
  ...
  long speed;
  if (!fp) {
    perror(i18n("Error:open network file failed"));
    exit(1);
  }
...
uint8_t *get_network_speed(const uint8_t *ifname, const uint8_t *txorrx,
  ...
  if (speed > 1000)
    sprintf(str, "%s: %.2f MB/s", prefix,
            ((float)end - start) / 1000.0 / 1000.0);
  else
...
uint8_t *get_network_total(const uint8_t *interface) {
  static uint8_t str[20];
  sprintf(str, "Total: %d MB",
          (get_network_status(interface, "tx") +
           get_network_status(interface, "rx")) /
              1024 / 1024);
  printf("%s\n", str);
  return str;
}
