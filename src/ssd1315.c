...
void ssd1315_init() {
  ...
  uint8_t init_seq[28] = {0xAE, 0x00, 0x10, 0x40, 0x81, 0xCF, 0xA1, 0xC8, 0xA6,
                          0xA8, 0x3F, 0xD3, 0x00, 0xD5, 0x80, 0xD9, 0xF1, 0xDA,
                          0x12, 0xDB, 0x40, 0x20, 0x02, 0x8D, 0x14, 0xA4, 0xA6};
...
void ssd1315_clear_buf() { memset(display_buffer, 0x00, 128 * 8); }

void ssd1315_clear_screen() { ssd1315_clear_buf(); }
...
void ssd1315_send_buf() {
  for (uint8_t i = 0; i < 8; i++) {
    ...
    for (uint8_t j = 0; j < 128; j++) buf[j] = display_buffer[j][i];
...
void ssd1315_fill_screen() { memset(display_buffer, 0xFF, 128 * 8); }
...
void ssd1315_draw_circle(uint16_t xc, uint16_t yc, uint16_t radius,
  ...
  if (filled) {
    while (x <= y) {
      for (yi = x; yi <= y; yi++) ssd1315_precircle(xc, yc, x, yi);
      if (d < 0)
        d = d + 4 * x + 6;
      else {
        d = d + 4 * (x - y) + 10;
